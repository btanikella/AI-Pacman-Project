...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        """
        #Updated By Bharadwaj Tanikella 2014
        pacmanPosition = gameState.getPacmanPosition()
        ...
        minimumDistance= 99999  #= 1e400  
        numGhost = 0 #iterations to upload the ghostValues List. For exampe [(0,position),(1,position),(2,position)], where the number represent the ghost itself. 
        ghostValues=[]
        ghostBelief=None
        bestPosition= None
        ...
        #Best Postion is needed to find, as well as the list is used to populate the data to provide the best position of the ghost, with respect to their index.
        for ghost in livingGhosts:
            for g in livingGhostPositionDistributions:
                ghostBelief=0
                for (pos, belief) in g.items():
                    if(ghostBelief< belief):
                        ghostBelief=belief
                        ghostValues.insert(numGhost,pos)
                numGhost+=1
        #Iterate through the ghostValues List to find if the distance is the best distance. If it is, make sure the values are assigned accordingly.
        for key in ghostValues:
            distance = self.distancer.getDistance(pacmanPosition,key)
            #If obviously the distance is smaller, change the min distance variable. 
            if(minimumDistance> distance):
                minimumDistance=distance
                bestPosition=key

        for legalAction in legal:
            tempDistance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition,legalAction),bestPosition)
