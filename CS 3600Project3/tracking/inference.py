...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            #When a ghost is captured the p value in the allPossible is set to be zero and the self.getJailPosition to be 1. 
            if (noisyDistance is None):
                allPossible[p]=0
                allPossible[self.getJailPosition()] =1.0  
            #emissionModel is greater than zero, the trueDistance*beliefs[position]
            elif emissionModel[trueDistance] > 0: allPossible[p] = emissionModel[trueDistance]*self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions: 
            #print self.getPositionDistribution(self.setGhostPosition(gameState,p)).items() 

            for pos, pr in self.getPositionDistribution(self.setGhostPosition(gameState, p)).items():
                allPossible[pos]=allPossible[pos]+(pr*self.beliefs[p])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in self.legalPositions:
            count = 0
            while count < (self.numParticles/len(self.legalPositions)):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for i in self.legalPositions:
                dist = util.manhattanDistance(i,pacmanPosition)
                allPossible[i]= allPossible[i]+emissionModel[dist]*self.getBeliefDistribution()[i]
            ...
            self.particles=[] # new list. Refresh the Particles List
    ...
    def getBeliefDistribution(self):
        ...
        for value in self.particles:
            beliefDist[value] = beliefDist[value]+1 # Increase 1, Act as a counter, Populating the temp dict.
        #self.beliefs.normalize()
        belifDist= beliefDist.normalize()
        return beliefDist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        #Cartesian Product. So essentially use Product function, function 
        currentList = []
        self.particles = []
        ...
        currentList = list(itertools.product(legal, repeat=num)) #Cartesian product of (legalPositions ... NumGhosts times.)
    
        #Random Shuffle when itertools is used.
